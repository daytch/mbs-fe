...
const EquipmentOHFunctionsPDF = (props) => {
  const { equipmentOHFunctions, project, projectRepresentation, showHeader, showFooter } = props
  console.log('equipmentOHFunctions:', equipmentOHFunctions)
  const HeaderTable = () => (
    <View
      style={{
        flexDirection: 'row',
    ...
    >
      ...
      <View style={{ marginLeft: 10, flexDirection: 'column', flex: 1 }}>
        <Text style={stylez.tableHeader}>OH Function</Text>
      </View>

      <View style={{ marginLeft: 10, width: 150 }}>
        <Text style={stylez.tableHeader}>Function Result</Text>
      </View>
    </View>
  )
  ...
  return (
    <Document>
      <Page style={styles.body}>
        ...
        {showHeader && Header('Equipment OH Functions')}

        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          <HeaderTable />
          {divider()}
          {equipmentOHFunctions.map((item, idx) => {
            return (
              <FourColumnLayout
                key={idx}
                no={idx + 1}
...
EquipmentOHFunctionsPDF.propTypes = {
  equipmentOHFunctions: PropTypes.array,
...
export default EquipmentOHFunctionsPDF
