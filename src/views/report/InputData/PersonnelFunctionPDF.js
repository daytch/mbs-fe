...
const PersonnelFunctionPDF = (props) => {
  const { personnelFunctions, project, projectRepresentation, showHeader, showFooter } = props
  console.log('personnelFunctions:', personnelFunctions)
  const HeaderTable = () => (
    <View
      style={{
        flexDirection: 'row',
    ...
    <Document>
      <Page style={styles.body}>
        ...
        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          <HeaderTable />
          {divider()}
          {personnelFunctions.map((item, idx) => {
            return (
              <FourColumnLayout
                key={idx}
                no={idx + 1}
...
PersonnelFunctionPDF.propTypes = {
...
export default PersonnelFunctionPDF
