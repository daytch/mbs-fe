...
const ServicesCostByCostCentre = (props) => {
    const {
    ...
    const listPeriods = dtServicesCostByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtServicesCostByCostCentre.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtServicesCostByCostCentre.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Services Cost By Cost Centre')}
...
ServicesCostByCostCentre.propTypes = {
    dtServicesCostByCostCentre: PropTypes.array,
...
export default ServicesCostByCostCentre
