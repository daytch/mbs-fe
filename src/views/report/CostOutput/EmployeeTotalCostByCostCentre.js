...
const EmployeeTotalCostByCostCentre = (props) => {
    const {
    ...
    const listPeriods = dtEmployeeTotalCostByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtEmployeeTotalCostByCostCentre.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtEmployeeTotalCostByCostCentre.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Employee Total Cost By Cost Centre')}
...
EmployeeTotalCostByCostCentre.propTypes = {
    dtEmployeeTotalCostByCostCentre: PropTypes.array,
...
export default EmployeeTotalCostByCostCentre
