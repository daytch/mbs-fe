...
const InfrastructureCostSummary = (props) => {
    ...
    const listPeriods = dtInfrastructureCostSummary[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtInfrastructureCostSummary.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtInfrastructureCostSummary.map((i, idx) => {
...
InfrastructureCostSummary.propTypes = {
    dtInfrastructureCostSummary: PropTypes.array,
...
export default InfrastructureCostSummary
