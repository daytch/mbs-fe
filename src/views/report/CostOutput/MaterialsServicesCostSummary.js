...
const MaterialsServicesCostSummary = (props) => {
    const {
    ...
    const listPeriods = dtMaterialsServicesCostSummary[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtMaterialsServicesCostSummary.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtMaterialsServicesCostSummary.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Materials/Services Cost Summary')}
...
MaterialsServicesCostSummary.propTypes = {
    dtMaterialsServicesCostSummary: PropTypes.array,
...
export default MaterialsServicesCostSummary
