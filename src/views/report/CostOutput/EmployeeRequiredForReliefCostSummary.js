...
const EmployeeRequiredForReliefCostSummary = (props) => {
    const {
    ...
    const listPeriods = dtEmployeeRequiredForReliefCostSummary[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtEmployeeRequiredForReliefCostSummary.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtEmployeeRequiredForReliefCostSummary.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Employee Required for Relief Cost Summary')}
...
EmployeeRequiredForReliefCostSummary.propTypes = {
    dtEmployeeRequiredForReliefCostSummary: PropTypes.array,
...
export default EmployeeRequiredForReliefCostSummary
