...
const EquipmentCommissioningByCostCentre = (props) => {
  const {
  ...
  const listPeriods = dtEquipmentCommissioningByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
    (x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    },
  )
  const listParent = []
  dtEquipmentCommissioningByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtEquipmentCommissioningByCostCentre.map((i, idx) => {
...
EquipmentCommissioningByCostCentre.propTypes = {
  dtEquipmentCommissioningByCostCentre: PropTypes.array,
...
export default EquipmentCommissioningByCostCentre
