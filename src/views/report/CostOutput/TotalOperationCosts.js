...
const TotalOperationCosts = (props) => {
    const {
    ...
    const listPeriods = dtTotalOperationCosts[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtTotalOperationCosts.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtTotalOperationCosts.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Total Operating Costs')}
...
TotalOperationCosts.propTypes = {
    dtTotalOperationCosts: PropTypes.array,
...
export default TotalOperationCosts
