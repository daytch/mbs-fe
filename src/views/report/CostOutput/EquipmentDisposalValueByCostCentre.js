...
const EquipmentDisposalValueByCostCentre = (props) => {
  ...
  let lastCol = []
  const listPeriods = dtEquipmentDisposalValueByCostCentre?.length
    ? dtEquipmentDisposalValueByCostCentre[0].rptEquipmentSchedulePeriodDtos.map((x) => {
        return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
      })
    : []
  const listParent = []
  dtEquipmentDisposalValueByCostCentre?.length &&
