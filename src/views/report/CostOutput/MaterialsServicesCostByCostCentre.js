...
const MaterialsServicesCostByCostCentre = (props) => {
    const {
    ...
    const listPeriods = dtMaterialsServicesCostByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtMaterialsServicesCostByCostCentre.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtMaterialsServicesCostByCostCentre.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Materials/Services Cost By Cost Centre')}
...
MaterialsServicesCostByCostCentre.propTypes = {
    dtMaterialsServicesCostByCostCentre: PropTypes.array,
...
export default MaterialsServicesCostByCostCentre
