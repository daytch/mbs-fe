...
const InfrastructureCostByCostCentre = (props) => {
    const {
    ...
    const listPeriods = dtInfrastructureCostByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtInfrastructureCostByCostCentre.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtInfrastructureCostByCostCentre.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Infrastructure Cost By Cost Centre')}
...
InfrastructureCostByCostCentre.propTypes = {
    dtInfrastructureCostByCostCentre: PropTypes.array,
...
export default InfrastructureCostByCostCentre
