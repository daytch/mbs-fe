...
const TotalProjectsCosts = (props) => {
    const {
    ...
    const listPeriods = dtTotalProjectsCosts[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtTotalProjectsCosts.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtTotalProjectsCosts.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Total Project Costs')}
...
TotalProjectsCosts.propTypes = {
    dtTotalProjectsCosts: PropTypes.array,
...
export default TotalProjectsCosts
