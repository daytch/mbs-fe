...
const MaterialsServicesCostNotConsumedByEquipment = (props) => {
    const {
    ...
    const listPeriods = dtMaterialsServicesCostNotConsumedByEquipment[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtMaterialsServicesCostNotConsumedByEquipment.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtMaterialsServicesCostNotConsumedByEquipment.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Materials/Services Cost (Not Consumed By Equipment)')}
...
MaterialsServicesCostNotConsumedByEquipment.propTypes = {
    dtMaterialsServicesCostNotConsumedByEquipment: PropTypes.array,
...
export default MaterialsServicesCostNotConsumedByEquipment
