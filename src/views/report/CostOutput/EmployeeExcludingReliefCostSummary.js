...
const EmployeeExcludingReliefCostSummary = (props) => {
    const {
    ...
    const listPeriods = dtEmployeeExcludingReliefCostSummary[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtEmployeeExcludingReliefCostSummary.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtEmployeeExcludingReliefCostSummary.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Employee Excluding Relief Cost Summary')}
...
EmployeeExcludingReliefCostSummary.propTypes = {
    dtEmployeeExcludingReliefCostSummary: PropTypes.array,
...
export default EmployeeExcludingReliefCostSummary
