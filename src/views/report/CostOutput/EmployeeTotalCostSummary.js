...
const EmployeeTotalCostSummary = (props) => {
    ...
    const listPeriods = dtEmployeeTotalCostSummary[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtEmployeeTotalCostSummary.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtEmployeeTotalCostSummary.map((i, idx) => {
...
EmployeeTotalCostSummary.propTypes = {
    dtEmployeeTotalCostSummary: PropTypes.array,
...
export default EmployeeTotalCostSummary
