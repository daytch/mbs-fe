...
const EmployeeExcludingReliefCostByCostCentre = (props) => {
    const {
    ...
    const listPeriods = dtEmployeeExcludingReliefCostByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtEmployeeExcludingReliefCostByCostCentre.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtEmployeeExcludingReliefCostByCostCentre.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Employee Excluding Relief Cost by Cost Centre')}
...
EmployeeExcludingReliefCostByCostCentre.propTypes = {
    dtEmployeeExcludingReliefCostByCostCentre: PropTypes.array,
...
export default EmployeeExcludingReliefCostByCostCentre
