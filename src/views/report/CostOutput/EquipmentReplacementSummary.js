...
const EquipmentReplacementSummary = (props) => {
  const {
  ...
  const listPeriods = dtEquipmentReplacementSummary[0].rptEquipmentSchedulePeriodSummaryDtos.map(
    (x) => {
        ...
        >
          ...
          {item.rptEquipmentSchedulePeriodSummaryDtos.map((i, idx) => (
            <View key={idx} style={{ flexDirection: 'row', flex: 1, marginRight: '5px' }}>
              <Text style={{ textAlign: 'right' }}>{i.value && i.value > 0 ? i.value : ''}</Text>
            </View>
          ))}
  ...
  return (
    <Document>
      <Page style={styles.body}>
        ...
        {showHeader && Header('Equipment Replacement Summary')}

        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          <HeaderTable col={lastColumn} />
          {dtEquipmentReplacementSummary.map((item, idx) => {
...
EquipmentReplacementSummary.propTypes = {
  dtEquipmentReplacementSummary: PropTypes.array,
...
export default EquipmentReplacementSummary
