...
const MaterialsServicesCostConsumedByEquipment = (props) => {
    const {
    ...
    const listPeriods = dtMaterialsServicesCostConsumedByEquipment[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtMaterialsServicesCostConsumedByEquipment.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtMaterialsServicesCostConsumedByEquipment.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Materials/Services Cost (Consumed By Equipment)')}
...
MaterialsServicesCostConsumedByEquipment.propTypes = {
    dtMaterialsServicesCostConsumedByEquipment: PropTypes.array,
...
export default MaterialsServicesCostConsumedByEquipment
