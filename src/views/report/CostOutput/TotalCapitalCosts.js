...
const TotalCapitalCosts = (props) => {
    ...
    const listPeriods = dtTotalCapitalCosts[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtTotalCapitalCosts.forEach((item, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Total Capital Costs')}
...
TotalCapitalCosts.propTypes = {
    dtTotalCapitalCosts: PropTypes.array,
...
export default TotalCapitalCosts
