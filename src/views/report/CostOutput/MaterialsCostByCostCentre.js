...
const MaterialsCostByCostCentre = (props) => {
    const {
    ...
    const listPeriods = dtMaterialsCostByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
        (x) => {
            return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
        },
    )
    const listParent = []
    dtMaterialsCostByCostCentre.forEach((item, idx) => {
    ...
    const BodyTable = ({ item }) => {
        return (
            <>
                ...
                {dtMaterialsCostByCostCentre.map((i, idx) => {
    ...
    return (
        <Document>
            <Page style={styles.body}>
                ...
                {showHeader && Header('Materials Cost By Cost Centre')}
...
MaterialsCostByCostCentre.propTypes = {
    dtMaterialsCostByCostCentre: PropTypes.array,
...
export default MaterialsCostByCostCentre
