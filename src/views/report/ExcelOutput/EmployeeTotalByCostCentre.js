...
const EmployeeTotalByCostCentre = ({ excelData, project, projectRepresentation }) => {
  const listPeriods = excelData[0]?.rptEmployeeSchedulePeriodDtos?.map((x) => {
  ...
  excelData.forEach((item, idx) => {
    ...
    const tempData = item.rptEmployeeSchedulePeriodDtos.map((item) => item.value)
    if (parentTemp === '') {
      ...
      parentTemp = item.costCentreCode
      listData.push([item.costCentreCode, item.employeeName, ...tempData.map(() => ''), ''])
    }
    listData.push(['', item.employeeName, ...tempData, Math.max(...tempData)])
  })
  ...
  FileSaver.saveAs(data, 'EmployeeTotalByCostCentre', +fileExtension)
}

export default EmployeeTotalByCostCentre
