...
const MaterialsServicesByCostCentre = (props) => {
  const {
  ...
  const listPeriods = dtMaterialsServicesByCostCentre[0]?.rptMaterialServiceSchedulePeriodDtos?.map(
    (x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    },
  )
  const listParent = []
  dtMaterialsServicesByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtMaterialsServicesByCostCentre.map((i, idx) => {
...
MaterialsServicesByCostCentre.propTypes = {
  dtMaterialsServicesByCostCentre: PropTypes.array,
...
export default MaterialsServicesByCostCentre
