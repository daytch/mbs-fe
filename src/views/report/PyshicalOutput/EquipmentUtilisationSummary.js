...
const EquipmentUtilisationSummary = (props) => {
  const { dtEquipmentUtilisationSummary, project, projectRepresentation, showHeader, showFooter } =
    props
    ...
    <Document>
      <Page style={styles.body}>
        ...
        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          <HeaderTable />
          {dtEquipmentUtilisationSummary.map((item, idx) => {
            return (
              <FourColumnLayout
                key={idx}
                no={idx + 1}
...
EquipmentUtilisationSummary.propTypes = {
  dtEquipmentUtilisationSummary: PropTypes.array,
...
export default EquipmentUtilisationSummary
