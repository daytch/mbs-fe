...
const EquipmentUtilisationByCostCentre = (props) => {
  const {
  ...
  const listPeriods = dtEquipmentUtilisationByCostCentre[0]?.rptEquipmentSchedulePeriodDtos?.map(
    (x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    },
  )
  const listParent = []
  dtEquipmentUtilisationByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtEquipmentUtilisationByCostCentre.map((i, idx) => {
...
EquipmentUtilisationByCostCentre.propTypes = {
  dtEquipmentUtilisationByCostCentre: PropTypes.array,
...
export default EquipmentUtilisationByCostCentre
