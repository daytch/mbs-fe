...
const EquipmentTotalDisposalSummary = (props) => {
  const {
    ...
    <Document>
      <Page style={styles.body}>
        ...
        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          <HeaderTable />
          {dtEquipmentTotalDisposalSummary.map((item, idx) => {
            return (
              <FourColumnLayout
                key={idx}
                no={idx + 1}
...
EquipmentTotalDisposalSummary.propTypes = {
  dtEquipmentTotalDisposalSummary: PropTypes.array,
...
export default EquipmentTotalDisposalSummary
