...
const EquipmentTotalDisposalByCostCentre = (props) => {
  const {
  ...
  const listPeriods = dtEquipmentTotalDisposalByCostCentre[0]?.rptEquipmentSchedulePeriodDtos?.map(
    (x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    },
  )
  const listParent = []
  dtEquipmentTotalDisposalByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtEquipmentTotalDisposalByCostCentre.map((i, idx) => {
...
EquipmentTotalDisposalByCostCentre.propTypes = {
  dtEquipmentTotalDisposalByCostCentre: PropTypes.array,
...
export default EquipmentTotalDisposalByCostCentre
