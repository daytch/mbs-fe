...
const EmployeeTotalByCostCentre = (props) => {
  const {
  ...
  const listPeriods = dtEmployeeTotalByCostCentre[0].rptEmployeeSchedulePeriodDtos.map((x) => {
    return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
  })
  const listParent = []
  dtEmployeeTotalByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtEmployeeTotalByCostCentre.map((i, idx) => {
  ...
  return (
    <Document>
      <Page style={styles.body}>
        ...
        {showHeader && Header('Employee Total By Cost Centre')}
...
EmployeeTotalByCostCentre.propTypes = {
  dtEmployeeTotalByCostCentre: PropTypes.array,
