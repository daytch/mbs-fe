...
const EquipmentDisposalNotRequiredByCostCentre = (props) => {
  const {
  ...
  const listPeriods =
    dtEquipmentDisposalNotRequiredByCostCentre[0]?.rptEquipmentSchedulePeriodDtos?.map((x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    })
  const listParent = []
  dtEquipmentDisposalNotRequiredByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtEquipmentDisposalNotRequiredByCostCentre.map((i, idx) => {
...
EquipmentDisposalNotRequiredByCostCentre.propTypes = {
  dtEquipmentDisposalNotRequiredByCostCentre: PropTypes.array,
