...
const EquipmentFleetSummary = (props) => {
  const { dtEquipmentFleetSummary, project, projectRepresentation, showHeader, showFooter } = props
    ...
    <Document>
      <Page style={styles.body}>
        ...
        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          <HeaderTable />
          {dtEquipmentFleetSummary.map((item, idx) => {
            return (
              <FourColumnLayout
                key={idx}
                no={idx + 1}
...
EquipmentFleetSummary.propTypes = {
  dtEquipmentFleetSummary: PropTypes.array,
