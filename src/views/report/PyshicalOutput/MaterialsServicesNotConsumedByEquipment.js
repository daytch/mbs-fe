...
const MaterialsServicesNotConsumedByEquipment = (props) => {
  const {
  ...
  const listPeriods =
    dtMaterialsNotConsumedByEquipment[0]?.rptMaterialServiceSchedulePeriodDtos?.map((x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    })
  const listParent = []
  dtMaterialsNotConsumedByEquipment.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtMaterialsNotConsumedByEquipment.map((i, idx) => {
...
MaterialsServicesNotConsumedByEquipment.propTypes = {
  dtMaterialsNotConsumedByEquipment: PropTypes.array,
