...
const EquipmentDisposalExpiredSummary = (props) => {
  const {
  ...
  const listPeriods =
    dtEquipmentDisposalExpiredSummary[0]?.rptEquipmentSchedulePeriodSummaryDtos?.map((x) => {
    ...
    <Document>
      <Page style={styles.body}>
        ...
        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          ...
          {dtEquipmentDisposalExpiredSummary.map((item, idx) => {
...
EquipmentDisposalExpiredSummary.propTypes = {
  dtEquipmentDisposalExpiredSummary: PropTypes.array,
...
export default EquipmentDisposalExpiredSummary
