...
const EmployeeRequiredReliefByCostCentre = (props) => {
  const {
  ...
  const listPeriods = dtEmployeeRequiredReliefByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
    (x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    },
  )
  const listParent = []
  dtEmployeeRequiredReliefByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtEmployeeRequiredReliefByCostCentre.map((i, idx) => {
...
EmployeeRequiredReliefByCostCentre.propTypes = {
  dtEmployeeRequiredReliefByCostCentre: PropTypes.array,
