...
const MaterialsServicesConsumedByEquipment = (props) => {
  const {
  ...
  const listPeriods = dtMaterialsConsumedByEquipment[0]?.rptMaterialServiceSchedulePeriodDtos?.map(
    (x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    },
  )
  const listParent = []
  dtMaterialsConsumedByEquipment.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtMaterialsConsumedByEquipment.map((i, idx) => {
...
MaterialsServicesConsumedByEquipment.propTypes = {
  dtMaterialsConsumedByEquipment: PropTypes.array,
