...
const MaterialsServicesSummary = (props) => {
  const {
  ...
  const listPeriods =
    dtMaterialsServicesSummary[0]?.rptMaterialServiceSchedulePeriodSummaryDtos?.map((x) => {
    ...
    return (
      <>
        ...
        >
          ...
          {item.rptMaterialServiceSchedulePeriodSummaryDtos.map((i, idx) => (
            <View key={idx} style={{ flexDirection: 'row', flex: 1 }}>
              <Text style={{ textAlign: 'left' }}>{i.value && i.value > 0 ? i.value : ''}</Text>
            </View>
          ))}

          <View style={{ flexDirection: 'column' }}>
            <Text>
              {item?.rptMaterialServiceSchedulePeriodSummaryDtos?.length > 0
                ? calculateLastCol[lastColumn](
                    item.rptMaterialServiceSchedulePeriodSummaryDtos,
                    decimalPoint,
                  )
                : ''}
            </Text>
  ...
  return (
    <Document>
      <Page style={styles.body}>
        ...
        {showHeader && Header('Materials / Services Summary')}

        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          <HeaderTable col={lastColumn} />
          {dtMaterialsServicesSummary.map((item, idx) => {
...
MaterialsServicesSummary.propTypes = {
  dtMaterialsServicesSummary: PropTypes.array,
...
export default MaterialsServicesSummary
