...
const EmployeeRequiredReliefSummary = (props) => {
  const {
    ...
    <Document>
      <Page style={styles.body}>
        ...
        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          <HeaderTable />
          {dtEmployeeRequiredReliefSummary.map((item, idx) => {
            return (
              <FourColumnLayout
                key={idx}
                no={idx + 1}
...
EmployeeRequiredReliefSummary.propTypes = {
  dtEmployeeRequiredReliefSummary: PropTypes.array,
...
export default EmployeeRequiredReliefSummary
