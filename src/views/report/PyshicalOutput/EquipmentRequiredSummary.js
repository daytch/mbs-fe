...
const EquipmentRequiredSummary = (props) => {
  const {
    /* dtEquipmentRequiredSummary,*/ project,
  ...
  return (
    <Document>
      <Page style={styles.body}>
        ...
        {showHeader && Header('Equipment Required Summary')}

        <View style={{ marginTop: showHeader ? 10 : 35 }}>
          {/* <HeaderTable />
          {dtEquipmentRequiredSummary.map((item, idx) => {
            return (
              <FourColumnLayout
                key={idx}
                no={idx + 1}
...
EquipmentRequiredSummary.propTypes = {
  dtEquipmentRequiredSummary: PropTypes.array,
...
export default EquipmentRequiredSummary
