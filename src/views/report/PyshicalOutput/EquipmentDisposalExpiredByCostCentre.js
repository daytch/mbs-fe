...
const EquipmentDisposalExpiredByCostCentre = (props) => {
  const {
  ...
  const listPeriods =
    dtEquipmentDisposalExpiredByCostCentre[0]?.rptEquipmentSchedulePeriodDtos?.map((x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    })
  const listParent = []
  dtEquipmentDisposalExpiredByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtEquipmentDisposalExpiredByCostCentre.map((i, idx) => {
...
EquipmentDisposalExpiredByCostCentre.propTypes = {
  dtEquipmentDisposalExpiredByCostCentre: PropTypes.array,
...
export default EquipmentDisposalExpiredByCostCentre
