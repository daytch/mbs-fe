...
const EmployeeExcludingReliefByCostCentre = (props) => {
  const {
  ...
  const listPeriods = dtEmployeeExcludingReliefByCostCentre[0]?.rptEmployeeSchedulePeriodDtos?.map(
    (x) => {
      return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
    },
  )
  const listParent = []
  dtEmployeeExcludingReliefByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtEmployeeExcludingReliefByCostCentre.map((i, idx) => {
...
EmployeeExcludingReliefByCostCentre.propTypes = {
  dtEmployeeExcludingReliefByCostCentre: PropTypes.array,
...
export default EmployeeExcludingReliefByCostCentre
