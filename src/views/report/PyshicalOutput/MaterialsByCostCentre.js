...
const MaterialsByCostCentre = (props) => {
  const {
  ...
  const listPeriods = dtMaterialsByCostCentre[0]?.rptMaterialServiceSchedulePeriodDtos?.map((x) => {
    return { periodId: x.periodId, positionN: x.positionN, periodName: x.periodName }
  })
  const listParent = []
  dtMaterialsByCostCentre.forEach((item, idx) => {
  ...
  const BodyTable = ({ item }) => {
    return (
      <>
        ...
        {dtMaterialsByCostCentre.map((i, idx) => {
...
MaterialsByCostCentre.propTypes = {
  dtMaterialsByCostCentre: PropTypes.array,
...
export default MaterialsByCostCentre
