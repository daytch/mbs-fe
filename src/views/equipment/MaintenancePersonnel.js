...
const EditableCell = ({
  ...
  const onChange = (e) => {
    ...
    updateMaintenancePersonnel(index, id, isi)
  }
  const onBlur = () => {
              ...
              <tr key={i} {...row.getRowProps()}>
                {row.cells.map((cell, idx) => {
                  return (
                    <td
                      // onClick={(e) => {
                      //   updateMaintenancePersonnel(e, row)
                      // }}
                      key={idx}
                      {...cell.getCellProps()}
                    >
...
const MaterialOrService = ({
  ...
  const columns = useMemo(
    () => [
      {
        Header: 'Employee Type (Maintenance)',
  ...
  const [data, setData] = useState(() => makeData(20, 'mainCost', dataMaintenancePersonnel))
