...
function Table({ updateOperators, employee, columns, data, updateMyData, skipPageReset }) {
  const {
    getTableProps,
    getTableBodyProps,
    headerGroups,
...
const Operators = ({ updateOperators, dataOperators, employee, isNew }) => {
  const columns = useMemo(
    () => [
      {
        Header: 'Employee Type (Operator)',
        accessor: 'employeeType',
      },
      {
        Header: 'No of Operators',
        accessor: 'totalOperators',
      },
    ],
    [],
  )

  const [data, setData] = useState(() => makeData(20, 'mainCost', dataOperators))
